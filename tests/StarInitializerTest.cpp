...
TEST_CASE("Test Copy Constructor", "[weight=1]") {
  StarInitializer test_initializer("test_data/test_data.csv", 200);

  StarInitializer new_intializer(test_initializer);
  
  // loaded_stars are the stars of the StarInitializer created by the copy constructor
  std::vector<Star*> loaded_stars = new_intializer.GetStarList();
...
TEST_CASE("Test Copy Assignment Operator", "[weight=1]") {
  StarInitializer test_initializer("test_data/test_data.csv", 200);
